...
export const AREA_TOOL_OPTIONS: Array<{ value: AreaTool; label: string; icon: string }> = [
  { value: "layers", label: "层级面板", icon: "▦" },
  { value: "canvas", label: "画布", icon: "⌗" },
  { value: "properties", label: "属性面板", icon: "☷" },
  { value: "overview", label: "场景总览", icon: "⊙" },
  { value: "bindings", label: "资源绑定", icon: "↔" },
  { value: "slice", label: "九宫格", icon: "▧" },
  { value: "slice-candidates", label: "九宫格候选", icon: "▤" },
  { value: "slice-marker", label: "九宫格标记", icon: "✣" },
  { value: "preview", label: "预览", icon: "◉" },
  { value: "export-targets", label: "导出目标", icon: "⇩" },
];
...
export function areaToolIcon(tool: AreaTool): string {
  return AREA_TOOL_OPTIONS.find((item) => item.value === tool)?.icon ?? "•";
}
  ...
  return <div className="area-toolbar" ref={rootRef} onPointerDown={(event) => event.stopPropagation()}>
    <div className="area-tool-picker">
      <button className="area-tool-picker-button" type="button" aria-label={`当前工具：${current.label}`} aria-haspopup="menu" aria-expanded={open}
        ...
        <span className="area-tool-icon" aria-hidden="true">{current.icon}</span>
      </button>
      {open && <div className="area-tool-picker-menu" role="menu" aria-label="区域工具">
        {AREA_TOOL_OPTIONS.map((item) => <button key={item.value} type="button" role="menuitem"
          ...
          <span className="area-tool-option-icon" aria-hidden="true">{item.icon}</span>
