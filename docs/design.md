...
| 能力 | 现状 |
|---|---|
| PSD 解析 | `psdImport.ts`,ag-psd `readPsd`;图层树、文本图层、图层样式(投影/描边)裁剪位图、矢量 shape 兜底栅格化、"9" 文件夹九宫格约定、文件夹名推断控件类型 |
...
| 工作流 | `WorkspaceTabs.tsx`:控件类型 → 九宫格 → 布局适配 → 动画(占位)→ 导出(占位);未标完控件类型时锁定后续工作区 |
...
```
控件类型 → 九宫格 → 布局适配 → 动画 → 验收预览 → 导出
```

- 控件类型:12 种标签(现状),未标完锁定后续工作区(现状)
- 动画:时间轴编辑器(见 §7)
- 验收预览:引擎模拟视图(见 §8)
- 导出:zip 下载(见 §9)
...
1. **控件类型手动标记交互(需求方反馈)**:PSD 未按约定命名时需手动在控件类型中标记,现有交互不佳,交互方案待需求方进一步考虑后确定
